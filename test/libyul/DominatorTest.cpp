...
BOOST_FIXTURE_TEST_CASE(check_dominance, DominatorFixture)
{
	...
	std::vector<std::vector<bool>> expectedDominanceByVertex = {
		makeDominanceVertexRelation(ranges::views::iota(0u, test.numVertices) | ranges::to<std::vector<size_t>>), // A dominates all vertices, including itself
		makeDominanceVertexRelation(ranges::views::iota(1u, test.numVertices) | ranges::to<std::vector<size_t>>), // B, C, D, E, F, G, H
	...
	solAssert(expectedDominanceByVertex.size() == test.numVertices);
	solAssert(test.vertices.size() == test.numVertices);
	...
	{
		solAssert(expectedDominanceByVertex[i].size() == test.numVertices);
