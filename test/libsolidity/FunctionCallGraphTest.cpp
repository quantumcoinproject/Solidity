...
#include <test/libsolidity/FunctionCallGraphTest.h>
...
TestCase::TestResult FunctionCallGraphTest::run(std::ostream& _stream, std::string const& _linePrefix, bool _formatted)
{
	compiler().reset();
	...
	for (auto [top, subs]: compiler().experimentalAnalysis().annotation<experimental::FunctionCallGraph>().functionCallGraph.edges)
	{
