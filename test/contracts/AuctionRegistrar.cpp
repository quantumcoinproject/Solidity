...
BOOST_AUTO_TEST_CASE(properties)
{
	...
	{
		m_sender = account(0);
		sendEther(account(count), u256(28) * ether);
		...
		registrar.setSubRegistrar(name, h160(addr + 28));
		BOOST_CHECK_EQUAL(registrar.subRegistrar(name), h160(addr + 28));
		...
		BOOST_CHECK_EQUAL(registrar.addr(name), h160(addr));
		registrar.setSubRegistrar(name, h160(addr + 28 + 1));
		BOOST_CHECK_EQUAL(registrar.subRegistrar(name), h160(addr + 28));
