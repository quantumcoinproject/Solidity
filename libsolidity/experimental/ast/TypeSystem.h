...
public:
	...
	bool isFixedTypeVar(TypeVariable const& _typeVariable) const { return m_fixedTypeVariables.count(_typeVariable.index()) != 0; }
	void fixTypeVars(std::vector<TypeVariable> const& _typeVariables);
	void generalizeTypeVars(std::vector<TypeVariable> const& _typeVariables);
