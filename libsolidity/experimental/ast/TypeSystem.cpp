...
{
	for (TypeVariable const& typeVar: TypeEnvironmentHelpers{*this}.typeVars(_type))
...
std::variant<TypeClass, std::string> TypeSystem::declareTypeClass(std::string _name, Declaration const* _declaration, bool _primitive)
{
	...
	Type maybeTypeVariable = (_primitive ? freshVariable({{typeClass}}) : freshTypeVariable({{typeClass}}));
