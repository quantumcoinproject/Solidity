...
void CompilerUtils::splitExternalFunctionType(bool _leftAligned)
{
	...
	else
	{
		...
		m_context << ((u256(1) << 224) - 1) << Instruction::AND << Instruction::SWAP1;
	}
...
void CompilerUtils::combineExternalFunctionType(bool _leftAligned)
{
	...
	if (!_leftAligned)
		m_context << ((u256(1) << 224) - 1) << Instruction::AND;
...
)
{
	...
	case Type::Category::FixedBytes:
	{
		...
		{
			solAssert(typeOnStack.numBytes() * 8 == 224, "");
			rightShiftNumberOnStack(256 - 224);
		}
		else
		{
	...
	case Type::Category::RationalNumber:
		if (targetTypeCategory == Type::Category::FixedBytes)
		{
			...
			if (auto typeOnStack = dynamic_cast<IntegerType const*>(&_typeOnStack))
			{
				...
				solAssert(targetBytesType.numBytes() * 8 == 224, "");
